...
bot.hears('CUR_Random Image', (message) =>{
  ...
  request('https://api.nasa.gov/mars-photos/api/v1/rovers/curiosity/photos?sol='+numbe+'&api_key=DEMO_KEY', { json: true }, (err, res, body) => {
  if (err) { return console.log(err); }
...
console.log(month)
 ...
 //https://api.nasa.gov/mars-photos/api/v1/rovers/curiosity/photos?sol='+number+'&camera=fhaz&api_key=DEMO_KEY
...
const ur2 = '&camera=';
const ur3 = '&api_key=xxxxxxxxxxxxxxxxx';
...
var randomsol;
  function custom_image_for_cam(cam, message)
  {
    ...
    if(cam = 'navcam')
    {
      final_url = 'https://api.nasa.gov/mars-photos/api/v1/rovers/curiosity/photos?sol='+numbe+'&camera='+cam+'&api_key=xxxxxxxxxxxxxx';
